...
field = arcpy.GetParameterAsText(2) #field that contains names of feature classes want to view (from ones held in workspace)("Name")
...
try:
    with arcpy.da.SearchCursor(shapeName, field) as cursor:
        for fc in cursor:
            ...
            arcpy.AddMessage("Feature class number {} added to the data frame".format(fcCount))
            fcCount += 1
